...
def compute_tau(limit):
    """Computes tau(n) (the number of divisors)."""
...
def worker(start, end, tau_cache, progress_queue, worker_id, verbose):
    ...
    for idx, n in enumerate(range(start, end + 1)):
        ...
        if tau_cache[1] * tau_cache[n] == target:
            types_found.add(5)

        for i, delta in enumerate(divs):
            for omega in divs[i:]:
                if tau_cache[delta] * tau_cache[omega] == target:
        ...
        if verbose:
            print(f"Worker {worker_id}: n={n:4d}, tau(n)={target:2d}, types={sorted(types_found)}")
...
def leikhmans_tau_theorem_parallel(max_n=10000000, limit=20000000, num_workers=None, verbose=False):
    start_time = time.time()

    print("Computing tau(n) using a linear sieve...")
    tau_cache = compute_tau(limit)
    ...
    args = [(s, e, tau_cache, progress_queue, wid, verbose) for (s, e, wid) in ranges]
    ...
    print("\n" + "=" * 60)
    print("Verification results for tau(n)")
    print("=" * 60)
...
if __name__ == "__main__":
    leikhmans_tau_theorem_parallel(max_n=10000000, limit=20000000, verbose=False)