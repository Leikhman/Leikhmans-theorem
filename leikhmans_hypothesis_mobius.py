...
def worker(start, end, mu_cache, progress_queue, worker_id, verbose):
    ...
    for idx, n in enumerate(range(start, end + 1)):
        ...
        if mu_cache[1] * mu_cache[n] == target:
            types_found.add(5)

        for i, delta in enumerate(divs):
            for omega in divs[i:]:
                if mu_cache[delta] * mu_cache[omega] == target:
        ...
        if verbose:
            print(f"Worker {worker_id}: n={n:4d}, mu(n)={target:2d}, types={sorted(types_found)}")
...
def leikhmans_mobius_theorem(max_n=10000000, limit=20000000, num_workers=None, verbose=False):
    start_time = time.time()

    print("Computing mu(n) using a linear sieve...")
    mu_cache = compute_mobius(limit)
    ...
    args = [(s, e, mu_cache, progress_queue, wid, verbose) for (s, e, wid) in ranges]
    ...
    print("\n" + "=" * 60)
    print("Verification results for mu(n)")
    print("=" * 60)
...
if __name__ == "__main__":
    leikhmans_mobius_theorem(max_n=10000000, limit=20000000, verbose=False)