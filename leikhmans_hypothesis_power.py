...
def compute_power(limit, k):
    """Computes f(n) = n^k for all n up to limit (64‑bit)."""
    power = array('Q', [0]) * (limit + 1)
    power[0] = 0
...
def worker(start, end, power_cache, progress_queue, worker_id, k, verbose):
    ...
    for idx, n in enumerate(range(start, end + 1)):
        target = power_cache[n]
        divs = divisors(n)
        types_found = set()

        if power_cache[1] * power_cache[n] == target:
            types_found.add(5)

        for i, delta in enumerate(divs):
            for omega in divs[i:]:
                if power_cache[delta] * power_cache[omega] == target:
        ...
        if verbose:
            print(f"Worker {worker_id}: n={n:4d}, {n}^{k}={target:>12}, types={sorted(types_found)}")
...
def leikhmans_power_theorem(max_n=10000000, limit=20000000, k=2, num_workers=None, verbose=False):
    start_time = time.time()

    print(f"Computing n^{k} for n up to {limit}...")
    power_cache = compute_power(limit, k)
    ...
    args = [(s, e, power_cache, progress_queue, wid, k, verbose) for (s, e, wid) in ranges]
    ...
    print("\n" + "=" * 60)
    print(f"Verification results for n^{k}")
    print("=" * 60)
...
if __name__ == "__main__":
    leikhmans_power_theorem(max_n=10000000, limit=20000000, k=2, verbose=False)