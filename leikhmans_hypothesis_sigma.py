...
def compute_sigma(limit):
    """Computes sigma(n) (sum of divisors)."""
    ...
    for i in range(2, limit + 1):
        ...
        for p in primes:
            ...
            spf[i * p] = p
            if i % p == 0:
                sigma[i * p] = sigma[i] * p + sigma[i // spf[i]]
                break
            else:
...
def worker(start, end, sigma_cache, progress_queue, worker_id, verbose):
    ...
    for idx, n in enumerate(range(start, end + 1)):
        target = sigma_cache[n]
        divs = divisors(n)
        types_found = set()

        if sigma_cache[1] * sigma_cache[n] == target:
            types_found.add(5)

        for i, delta in enumerate(divs):
            for omega in divs[i:]:
                if sigma_cache[delta] * sigma_cache[omega] == target:
        ...
        if verbose:
            print(f"Worker {worker_id}: n={n:4d}, sigma(n)={target:3d}, types={sorted(types_found)}")
...
def leikhmans_sigma_theorem(max_n=10000000, limit=20000000, num_workers=None, verbose=False):
    start_time = time.time()

    print("Computing sigma(n) using a linear sieve...")
    sigma_cache = compute_sigma(limit)
    ...
    args = [(s, e, sigma_cache, progress_queue, wid, verbose) for (s, e, wid) in ranges]
    ...
    print("\n" + "=" * 60)
    print("Verification results for sigma(n)")
    print("=" * 60)
...
if __name__ == "__main__":
    leikhmans_sigma_theorem(max_n=10000000, limit=20000000, verbose=False)